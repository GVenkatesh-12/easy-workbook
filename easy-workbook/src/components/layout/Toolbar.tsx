...
export function Toolbar() {
  ...
  return (
    <header className="flex flex-wrap items-center gap-4 px-4 py-3 glass border-b border-surface-700/50 z-30 shrink-0 lg:flex-nowrap lg:gap-5 lg:px-6">
      {/* Left: Logo + file info */}
      <div className="flex min-w-0 flex-1 items-center gap-4 lg:flex-none">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-indigo-600 flex items-center justify-center shrink-0">
      ...
      {pdfFile && (
        <div className="order-3 flex w-full min-w-0 items-center justify-start gap-4 overflow-x-auto pt-2 lg:order-none lg:w-auto lg:flex-1 lg:justify-center lg:overflow-visible lg:pt-0">
          {/* Mode toggle group */}
          <div className="flex shrink-0 items-center gap-1 rounded-xl bg-surface-800/80 p-1">
            <IconButton
              size="sm"
              active={mode === 'view'}
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            <button
              onClick={resetZoom}
              className="min-w-[56px] shrink-0 rounded-xl px-3 py-2 text-center font-mono text-xs font-medium text-surface-300 transition-colors hover:bg-surface-800 hover:text-surface-100"
            >
              {Math.round(zoom * 100)}%
            </button>
        ...
        {pdfFile && (
          <>
            ...
            >
              ...
              {questionCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-brand-500 rounded-full text-[10px] font-bold text-white flex items-center justify-center ring-2 ring-surface-950">
